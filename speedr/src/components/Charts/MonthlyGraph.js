...
class MonthlyGraph extends React.Component {
  ...
  componentDidMount = () => {
    ...
    //   hashTable[texts[j].name] = 0;
    // }

    // for (let i = 0; i < resultOfWeek.length; i++) {
    //   for (let j = 0; j < texts.length; j++) {
    //     if (resultOfWeek[i].text_id === texts[j].id) {
    //       if (hashTable[texts[j].name]) {
    //         hashTable[texts[j].name] += 1;
    //       } else {
    //         hashTable[texts[j].name] = 1;
    //       }
    //     }
